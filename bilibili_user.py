...
def getsource(url):
	...
	head = {
		'User-Agent': ua,
		'Referer': 'https://space.bilibili.com/' + str(100) + '?from=search&seid=' + str(random.randint(10000, 50000))
	}
	jscontent = requests \
		.session() \
	...
	try:
		...
		if statusJson == True:
			if 'data' in jsDict.keys():
				...
				try:
					res = requests.get(
						'https://api.bilibili.com/x/relation/stat?vmid=306805&jsonp=jsonp').text
					js_fans_data = json.loads(res)
					return str(js_fans_data['data']['follower'])
				except Exception as e:
					print("errasdsaor:"+str(e))
					fans = 0
					archiveview = 0
					article = 0
			else:
...
if __name__ == "__main__":
    print(getsource('https://space.bilibili.com/306805'+sys.argv[1]))
